...
import com.foodorder.paymentservice.dto.StatusUpdateDTO;
...
public class OrderServiceClient {
    ...
    public void updateOrderStatus(Long orderId, String status) {
        try {
            ...
            log.info("Order {} status updated to {} via order-service", orderId, status);
        } catch (RestClientException ex) {
            log.error("Failed to update order {} status to {}: {}", orderId, status, ex.getMessage());
            throw new IllegalStateException("Unable to reach order-service to update order status", ex);
        }
    }
}
